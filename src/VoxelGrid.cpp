#include "ndt_matching/common_includes.h"
...
void VoxelGrid<PointSourceType>::initialize()
{
	...
	centroid_ = std::make_shared<std::vector<Eigen::Vector3d> >(voxel_num_);

	icovariance_.reset();
	// icovariance_ = boost::make_shared<std::vector<Eigen::Matrix3d> >(voxel_num_);
	icovariance_ = std::make_shared<std::vector<Eigen::Matrix3d> >(voxel_num_);

	points_id_.reset();
	// points_id_ = boost::make_shared<std::vector<std::vector<int> > >(voxel_num_);
	points_id_ = std::make_shared<std::vector<std::vector<int> > >(voxel_num_);

	points_per_voxel_.reset();
	// points_per_voxel_ = boost::make_shared<std::vector<int> >(voxel_num_, 0);
	points_per_voxel_ = std::make_shared<std::vector<int> >(voxel_num_, 0);

	tmp_centroid_.reset();
	// tmp_centroid_ = boost::make_shared<std::vector<Eigen::Vector3d> >(voxel_num_);
	tmp_centroid_ = std::make_shared<std::vector<Eigen::Vector3d> >(voxel_num_);

	tmp_cov_.reset();
	// tmp_cov_ = boost::make_shared<std::vector<Eigen::Matrix3d> >(voxel_num_);
	tmp_cov_ = std::make_shared<std::vector<Eigen::Matrix3d> >(voxel_num_);
}
													...
													float min_x, float min_y, float min_z)
{
	...
	if (new_max_x > max_x_ || new_max_y > max_y_ || new_max_z > max_z_ ||
		...
		if (max_b_x > max_b_x_ || max_b_y > max_b_y_ || max_b_z > max_b_z_ ||
			...
			std::shared_ptr< std::vector<Eigen::Vector3d> > old_centroid = centroid_;
			std::shared_ptr< std::vector<Eigen::Matrix3d> > old_icovariance = icovariance_;
			std::shared_ptr< std::vector<std::vector<int> > > old_points_id = points_id_;
			std::shared_ptr< std::vector<int> > old_points_per_voxel = points_per_voxel_;
			...
			std::shared_ptr< std::vector<Eigen::Vector3d> > old_tmp_centroid = tmp_centroid_;
			std::shared_ptr< std::vector<Eigen::Matrix3d> > old_tmp_cov = tmp_cov_;
			...
			centroid_ = std::make_shared<std::vector<Eigen::Vector3d> >(voxel_num);
			icovariance_ = std::make_shared<std::vector<Eigen::Matrix3d> >(voxel_num);
			points_id_ = std::make_shared<std::vector<std::vector<int> > >(voxel_num);
			points_per_voxel_ = std::make_shared<std::vector<int> >(voxel_num, 0);
			tmp_centroid_ = std::make_shared<std::vector<Eigen::Vector3d> >(voxel_num);
			tmp_cov_ = std::make_shared<std::vector<Eigen::Matrix3d> >(voxel_num);
